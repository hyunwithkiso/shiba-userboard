  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.log(`[ChatTitleAPI] User board item creation notified for user ${gameUserId}, insert_id: ${result.id}`);
      } catch (refreshError) {
        console.error("[ChatTitleAPI] Error notifying user board item creation:", refreshError);
